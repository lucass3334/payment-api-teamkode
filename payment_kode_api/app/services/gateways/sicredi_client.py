...
async def get_access_token(empresa_id: str, retries: int = 2):
    """Obtém um token OAuth2 para autenticação com a API Pix do Sicredi, garantindo tentativas de fallback."""
    ...
    # ✅ Correção: Autenticação correta com base64 encoding
    ...
    # ✅ Correção: Verificar certificados antes de prosseguir
    ...
    async with httpx.AsyncClient(cert=(cert_files["sicredi_cert_base64"], cert_files["sicredi_key_base64"]), 
        ...
        for attempt in range(retries):
            try:
                ...
                logger.error(f"Erro HTTP na autenticação do Sicredi (tentativa {attempt+1}): {e.response.status_code} - {e.response.text}")
                if e.response.status_code in {401, 403}:  # Credenciais inválidas
                    raise

            except httpx.RequestError as e:
                logger.warning(f"Erro de conexão ao autenticar no Sicredi (tentativa {attempt+1}): {e}")

            await asyncio.sleep(2)  # Pequeno delay antes da nova tentativa

    raise RuntimeError(f"Falha ao obter token do Sicredi para empresa {empresa_id} após {retries} tentativas")

async def create_sicredi_pix_payment(empresa_id: str, amount: float, chave_pix: str, txid: str):
    """Cria um pagamento Pix no Sicredi, com manuseio de erros e validações."""
...
async def register_sicredi_webhook(empresa_id: str, chave_pix: str):
    """Registra o webhook do Sicredi para notificações de pagamento."""
